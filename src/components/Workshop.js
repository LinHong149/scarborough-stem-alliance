...
const WorkshopItem = ({ isActive, isSchool, inView }) => {
    ...
    return (
        <div className={`Workshop-topic rounded-50 p-64 w-full flex gap-25 items-center ${isActive ? active : notActive }`}>
            <FontAwesomeIcon className={`Workshop-icon h3 ${isActive ? 'text-white' : inView ? 'text-white' : 'text-black'  }`} icon={isSchool? faSchool : faBook }/>
            <h3 className={`Workshop-name h3 ${isActive ? 'text-white' : inView ? 'text-white' : 'text-black' }`}>{isSchool? 'Feeder School': 'TPL'}</h3>
        </div>
    )
}
    ...
    return (
        <div className={`Workshop-section flex gap-25 h-min border-grey items-center`}>
            <div className={`Workshop-nav w-1/2 flex flex-col gap-16 h-fit`}>
                <WorkshopItem isActive={true} isSchool={true} inView={inView}/>
                <WorkshopItem isActive={false} isSchool={false} inView={inView}/>
            </div>
